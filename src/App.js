...
const nicaraguaCities = [
  ...
  "Paiwas", "Posoltega", "Potosí", "Pozo de Oropéndola", "Prinzapolka", "Pueblo Nuevo",
...
const BriefcaseIcon = () => (<svg xmlns="http://www.w.org/2000/svg" className="h-16 w-16 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>);
...
function Header({ user, onLogin, onLogout, setView, goHome, notificationCount }) { /* ... same as before ... */ return ( <header className="bg-white shadow-md sticky top-0 z-50"><nav className="container mx-auto px-4 py-3 flex justify-between items-center"><div className="flex items-center cursor-pointer" onClick={goHome}><span className="text-2xl font-bold text-blue-600">Mercado<span className="text-sky-500">Nica</span></span></div><div className="flex items-center space-x-4">{user && <div className="cursor-pointer" onClick={() => setView({ page: 'messages' })}><BellIcon hasNotification={notificationCount > 0} /></div>}{user ? (<div className="relative group"><img src={user.photoURL || `https://i.pravatar.cc/150?u=${user.uid}`} alt="Perfil" className="w-10 h-10 rounded-full cursor-pointer" /><div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20 hidden group-hover:block"><span className="block px-4 py-2 text-sm text-gray-700 font-semibold truncate">{user.displayName}</span><a href="#" onClick={() => setView({ page: 'profile' })} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Mi Perfil</a><a href="#" onClick={onLogout} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Cerrar Sesión</a></div></div>) : ( <button onClick={onLogin} className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">Iniciar Sesión</button> )}</div></nav></header> ); }
function Footer() { /* ... same as before ... */ return ( <footer className="bg-white mt-12 py-6 border-t"><div className="container mx-auto text-center text-gray-600"><p>&copy; {new Date().getFullYear()} MercadoNica. Todos los derechos reservados.</p></div></footer> ); }
function HomePage({ setView }) { /* ... same as before ... */ return (<div className="container mx-auto"><div className="bg-white p-6 rounded-lg shadow-lg mb-8 text-center"><h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-2">Bienvenido a MercadoNica</h1><p className="text-gray-600 text-lg">¿Qué estás buscando hoy?</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div onClick={() => setView({ page: 'listings', type: 'empleo' })} className="bg-white p-8 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer flex flex-col items-center text-center"><BriefcaseIcon /><h2 className="text-2xl font-bold mt-4 text-gray-800">Empleos</h2><p className="text-gray-600 mt-2">Encuentra tu próximo trabajo o publica una vacante.</p></div><div onClick={() => setView({ page: 'listings', type: 'producto' })} className="bg-white p-8 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer flex flex-col items-center text-center"><TagIcon /><h2 className="text-2xl font-bold mt-4 text-gray-800">Artículos</h2><p className="text-gray-600 mt-2">Compra y vende productos nuevos o de segunda mano.</p></div></div></div>); }
function ListingsPage({ type, setView, navigateToMessages, currentUser }) { /* ... same as before with pagination ... */ const [listings, setListings] = useState([]); const [lastDoc, setLastDoc] = useState(null); const [loading, setLoading] = useState(true); const [hasMore, setHasMore] = useState(true); const fetchListings = async (afterDoc = null) => { setLoading(true); try { const listingsRef = collection(db, "listings"); let q = query(listingsRef, where("type", "==", type), orderBy("createdAt", "desc"), limit(8)); if (afterDoc) { q = query(listingsRef, where("type", "==", type), orderBy("createdAt", "desc"), startAfter(afterDoc), limit(8)); } const documentSnapshots = await getDocs(q); const newlistings = documentSnapshots.docs.map(doc => ({ id: doc.id, ...doc.data() })); const lastVisible = documentSnapshots.docs[documentSnapshots.docs.length - 1]; setListings(prev => afterDoc ? [...prev, ...newlistings] : newlistings); setLastDoc(lastVisible); if (documentSnapshots.docs.length < 8) { setHasMore(false); } } catch (error) { console.error("Error fetching listings:", error); } setLoading(false); }; useEffect(() => { fetchListings(); }, [type]); return ( <div className="container mx-auto"><div className="flex justify-between items-center mb-8"><h1 className="text-3xl font-bold">{type === 'empleo' ? 'Empleos' : 'Artículos'}</h1><button onClick={() => setView({ page: 'publish', type: type })} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Publicar {type === 'empleo' ? 'Empleo' : 'Artículo'}</button></div><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">{listings.map(listing => <ListingCard key={listing.id} listing={listing} navigateToMessages={navigateToMessages} currentUser={currentUser} />)}</div>{loading && <p className="text-center mt-8">Cargando...</p>}{hasMore && !loading && listings.length > 0 && ( <div className="text-center mt-8"><button onClick={() => fetchListings(lastDoc)} className="bg-gray-700 text-white px-6 py-3 rounded-lg hover:bg-gray-800">Cargar Más</button></div> )}{!loading && listings.length === 0 && <p className="text-center text-gray-500 mt-8">No hay anuncios en esta categoría.</p>}</div> ); }
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105 relative">
            ...
            {listing.photos && listing.photos.length > 1 && (
                <div className="absolute top-2 right-2 bg-black bg-opacity-60 text-white text-xs font-bold px-2 py-1 rounded-md flex items-center">
                    <CameraIcon />
                    <span className="ml-1">1/{listing.photos.length}</span>
                </div>
            )}
            <div className="p-4">
...
function PublishPage({ type, setView, user }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.title || !location || imageFiles.length === 0) {
            alert("Por favor, completa el título, la ubicación y sube al menos una imagen.");
            return;
        }
    ...
    return (
        <div className="container mx-auto max-w-2xl"><div className="bg-white p-8 rounded-lg shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <textarea placeholder="Descripción detallada" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
                    ...
                    <div className="mt-2 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                        {previews.map((preview, index) => (
                            <div key={index} className="relative">
                                ...
                                <button type="button" onClick={() => removeImage(index)} className="absolute top-0 right-0 bg-red-500 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs">&times;</button>
                            </div>
                        ))}
